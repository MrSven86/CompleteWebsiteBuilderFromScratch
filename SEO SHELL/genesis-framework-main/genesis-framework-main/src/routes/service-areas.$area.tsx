...
export const AREAS = [
  { slug: "downtown", name: "Area 1" },
  { slug: "north-side", name: "Area 2" },
  { slug: "south-side", name: "Area 3" },
  { slug: "east-end", name: "Area 4" },
  { slug: "west-end", name: "Area 5" },
];
...
export const Route = createFileRoute("/service-areas/$area")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.name ?? "Service Area"} — MyLogo` },
      { name: "description", content: `Professional services in ${loaderData?.name ?? ""}.` },
      { property: "og:title", content: `${loaderData?.name ?? "Service Area"} — MyLogo` },
      { property: "og:description", content: `Professional services in ${loaderData?.name ?? ""}.` },
    ],
  }),
  notFoundComponent: () => (
  ...
  errorComponent: ({ error }) => <div style={{ padding: 80 }}>{error.message}</div>,
  component: AreaPage,
});
...
export function ServiceAreasNavDropdown({ left }: { left: number }) {
  const [open, setOpen] = useState(false);
  return (
    <div
      onMouseEnter={() => setOpen(true)}
    ...
    <div className="canvas-1920-wrap" style={{ width: "100%", height: wrapHeight, overflow: "hidden", background: "white" }}>
      <div className="canvas-1920" style={{ height: canvasH, background: "white", transform: `scale(${scale})` }}>
        ...
        <div style={{ width: 1920, height: 320, left: 0, top: 163, position: "absolute", background: "#27272A", overflow: "hidden" }}>
          ...
          <Link to="/service-areas" style={{ position: "absolute", left: 192, top: 14, color: "white", fontSize: 14, fontFamily: NS, textDecoration: "none" }}>Service Areas</Link>
          <div style={{ position: "absolute", left: 305, top: 22, color: "white", fontFamily: NS }}>›</div>
          <div style={{ position: "absolute", left: 324, top: 14, color: "white", fontSize: 14, fontFamily: NS, fontWeight: 600 }}>{area.name}</div>
          ...
          <div style={{ width: 1049, position: "absolute", left: 116, top: 187, color: "white", fontSize: 20, fontFamily: O, lineHeight: "32px" }}>
            Detail about service in {area.name}
          </div>
        </div>
        ...
        <div style={{ width: 752, position: "absolute", left: 270, top: 638, color: "black", fontSize: 40, fontFamily: O, fontWeight: 700, lineHeight: "48px" }}>
          A headline about what we<br />provide in {area.name}
        </div>
        ...
        <div style={{ position: "absolute", left: 270, top: 1560, color: "black", fontSize: 22, fontFamily: O, fontWeight: 700 }}>Other services in this service area</div>
        <div style={{ position: "absolute", left: 270, top: 1610, display: "flex", flexDirection: "column", gap: 10, color: "#1F2937", fontSize: 16, fontFamily: O }}>
          {SERVICES.map((s) => (
            <Link key={s.slug} to="/services/$slug" params={{ slug: s.slug }} style={{ color: "#1F2937", textDecoration: "none" }}>{s.name}</Link>
          ))}
        </div>
        <div style={{ width: 538, height: 460, left: 1139, top: 1300, position: "absolute", background: "#D4D4D8", display: "flex", alignItems: "center", justifyContent: "center", color: "black", fontSize: 24, fontFamily: O }}>
          N/A
        </div>
        ...
        <div style={{ width: 1920, height: 320, left: 0, top: 1900, position: "absolute", background: "white" }}>
          <div style={{ width: 1385, height: 218.25, left: 260, top: 40, position: "absolute" }}>
            <div style={{ position: "absolute", left: 24, top: 0, width: 180 }}>
              <div style={{ position: "absolute", left: 22, top: 8, color: "#1F2937", fontSize: 20, fontFamily: O, fontWeight: 700, letterSpacing: 0.55 }}>EXCELLENT</div>
        ...
        <div style={{ width: 1786, height: 971, left: 31, top: 3260, position: "absolute" }}>
          <div style={{ position: "absolute", left: 130, top: 0, color: "black", fontSize: 16, fontFamily: O, textTransform: "uppercase", lineHeight: "22.5px" }}>GET IT TOUCH WITH US</div>
        ...
        <div style={{ width: 1920, height: 928, left: 0, top: 4376, position: "absolute", background: "#525252" }}>
          ...
          <div style={{ position: "absolute", left: 1432, top: 278, display: "flex", flexDirection: "column", gap: 16 }}>
            {AREAS.map((a) => (
              <Link key={a.slug} to="/service-areas/$area" params={{ area: a.slug }} style={{ color: "white", fontSize: 16, fontFamily: O, textDecoration: "none" }}>{a.name}</Link>
            ))}
          </div>
        </div>
